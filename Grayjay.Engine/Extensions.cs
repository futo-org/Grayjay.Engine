...
namespace Grayjay.Engine
{
    ...
    {
        public static T ToV8ValueObjectBlocking<T>(this IJavaScriptObject obj, PluginConfig config) => (T)obj.ToV8ValueObjectBlocking(config);
        public static object ToV8ValueObjectBlocking(this IJavaScriptObject obj, PluginConfig config)
        {
            ...
            obj.InvokeMethod("then",
                ...
                (object rejected) => {
                    ...
                    {
                        ex = GrayjayPlugin.GetExceptionFromV8(config, rejectedJs);
                        ev.Set();
                    }
                    else
                    {
        ...
        public static Task<T> ToV8ValueObjectAsync<T>(this IJavaScriptObject obj, PluginConfig config)
        {
            if (obj.Kind != JavaScriptObjectKind.Promise)
                throw new InvalidOperationException("Object is not a promise");
        ...
        public static Task<T> ToV8ValueObjectAsync<T>(this IJavaScriptObject obj, PluginConfig config, out V8PromiseMetadata promiseMeta)
        {
            if (obj.Kind != JavaScriptObjectKind.Promise)
                throw new InvalidOperationException("Object is not a promise");
        ...
        public static object InvokeV8(this IJavaScriptObject obj, PluginConfig config, string method, params object[] args)
        {
            ...
            if (result is IJavaScriptObject jresult && jresult.Kind == JavaScriptObjectKind.Promise)
                return jresult.ToV8ValueObjectBlocking(config);
            return result;
        }
        public static T InvokeV8<T>(this IJavaScriptObject obj, PluginConfig config, string method, params object[] args)
        {
            var plugin = GrayjayPlugin.GetEnginePlugin(obj.Engine);
            var result = (plugin != null) ? plugin.InterceptExceptions(() => obj.InvokeMethod(method, args)) :
                obj.InvokeMethod(method, args);
            if (result is IJavaScriptObject jresult && jresult.Kind == JavaScriptObjectKind.Promise)
                return (T)jresult.ToV8ValueObjectBlocking(config);
            return (T)result;
        }
        public static Task<object> InvokeV8Async(this IJavaScriptObject obj, PluginConfig config, string method, params object[] args)
        {
            var plugin = GrayjayPlugin.GetEnginePlugin(obj.Engine);
            var result = (plugin != null) ? plugin.InterceptExceptions(() => obj.InvokeMethod(method, args)) :
                obj.InvokeMethod(method, args);
            ...
            if (result is IJavaScriptObject jresult && jresult.Kind == JavaScriptObjectKind.Promise)
                return jresult.ToV8ValueObjectAsync<object>(config);
            return Task.FromResult(result);
        }
        public static Task<T> InvokeV8Async<T>(this IJavaScriptObject obj, PluginConfig config, string method, params object[] args)
        {
            var plugin = GrayjayPlugin.GetEnginePlugin(obj.Engine);
            var result = (plugin != null) ? plugin.InterceptExceptions(() => obj.InvokeMethod(method, args)) :
                obj.InvokeMethod(method, args);
            if (result is IJavaScriptObject jresult && jresult.Kind == JavaScriptObjectKind.Promise)
                return jresult.ToV8ValueObjectAsync<T>(config);
            return Task.FromResult((T)result);
        }
        public static Task<object> InvokeV8Async(this IJavaScriptObject obj, PluginConfig config, string method, out V8PromiseMetadata? promiseMeta, params object[] args)
        {
            var plugin = GrayjayPlugin.GetEnginePlugin(obj.Engine);
            var result = (plugin != null) ? plugin.InterceptExceptions(() => obj.InvokeMethod(method, args)) :
                obj.InvokeMethod(method, args);
            ...
            if (result is IJavaScriptObject jresult && jresult.Kind == JavaScriptObjectKind.Promise)
                return jresult.ToV8ValueObjectAsync<object>(config, out promiseMeta);
            promiseMeta = null;
            return Task.FromResult(result);
        }
        public static Task<T> InvokeV8Async<T>(this IJavaScriptObject obj, PluginConfig config, string method, out V8PromiseMetadata? promiseMeta, params object[] args)
        {
            var plugin = GrayjayPlugin.GetEnginePlugin(obj.Engine);
            var result = (plugin != null) ? plugin.InterceptExceptions(() => obj.InvokeMethod(method, args)) :
                obj.InvokeMethod(method, args);
            if (result is IJavaScriptObject jresult && jresult.Kind == JavaScriptObjectKind.Promise)
                return jresult.ToV8ValueObjectAsync<T>(config, out promiseMeta);
            promiseMeta = null;
            return Task.FromResult((T)result);
        }
        ...
        public static T GetOrThrow<T>(this IJavaScriptObject obj, GrayjayPlugin plugin, string key, string contextName, bool nullable)
        {
            ...
            if (val is Undefined)
                throw new InvalidOperationException($"Undefined property [{contextName}.{key}]");
            return (T)V8Converter.ConvertValue(plugin, typeof(T), val);
        }
        public static T GetOrDefault<T>(this IJavaScriptObject obj, GrayjayPlugin plugin, string key, string contextName, T def)
        {
            object val = obj.GetProperty(key);
            if (val is Undefined)
                return def;
        ...
        public static bool HasFunction(this IJavaScriptObject obj, string key)
        {
            var val = obj.GetProperty(key);
            if (val is IJavaScriptObject jsVal && jsVal.Kind == JavaScriptObjectKind.Function)
                return true;
            return false;
        }

        public static Dictionary<string, B> ToDictionary<B>(this ExpandoObject dynObj)
        {
            var dictionary = new Dictionary<string, B>();
            foreach (PropertyDescriptor propertyDescriptor in TypeDescriptor.GetProperties(dynObj))
            {
                object obj = propertyDescriptor.GetValue(dynObj);

                if (obj is B)
                    dictionary.Add(propertyDescriptor.Name, (B)(obj));
                else if (typeof(B) == typeof(string))
                    dictionary.Add(propertyDescriptor.Name, (B)(obj.ToString() as object));
                else
                    throw new NotImplementedException($"No conversion {obj.GetType().Name} => {typeof(B).Name}");
            }
            return dictionary;
        }
        public static Dictionary<string, B> ToDictionary<B>(this IScriptObject dynObj)
        {
            var dictionary = new Dictionary<string, B>();
            foreach (string name in dynObj.PropertyNames)
            {
                object obj = dynObj[name];

                if (obj is B)
                    dictionary.Add(name, (B)(obj));
                else if (typeof(B) == typeof(string))
                    dictionary.Add(name, (B)(obj.ToString() as object));
                else
                    throw new NotImplementedException($"No conversion {obj.GetType().Name} => {typeof(B).Name}");
            }
            return dictionary;
        ...
        public static MultiOrderedPager<T> CombineOrdered<T>(this IPager<T> pager, IPager<T> pager2, Func<T, T, bool> isBetterThan, bool allowFailure, int pageSize = 9)
        {
            return new MultiOrderedPager<T>(new List<IPager<T>>()
            {
                pager, pager2
            }, isBetterThan, allowFailure, pageSize);
        }
        public static IPager<T> CombineDistributed<T>(this IPager<T> pager, IPager<T> pager2, Func<T, T, bool> isBetterThan, bool allowFailure, int pageSize = 9)
        {
            return new MultiDistributionPager<T>(new List<IPager<T>>()
            {
                pager, pager2
            }.ToDictionary(x=>x, y=>1f), allowFailure, pageSize);
        }

        public static ReusablePager<T> AsReusable<T>(this IPager<T> pager)
        {
            return new ReusablePager<T>(pager);
        }
        ...
        public static string ToCamelCased(this string str)
        {
            ...
            if (capLength > 0)
                return str.Substring(0, capLength).ToLower() + str.Substring(capLength);
            return str;
        }

        private static HashSet<string> _slds = new HashSet<string>(new List<string>()
        {
        ...
        public static bool IsSLD(this string domain)
        {
            domain = "." + domain.ToLower().TrimStart('.');
            return _slds.Contains(domain);
        }
        ...
        public static JSList<T> ToJSCompatibleCollection<T>(this IEnumerable<T> arr)
        {
            return new JSList<T>(arr);
        }
        public class JSList<T>: List<T>
        {
            ...
            public JSList(IEnumerable<T> arr): base(arr)
            {
